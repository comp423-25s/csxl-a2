...
Revision ID: 1f1855f3cdda
Revises: 
Create Date: 2025-05-05 13:51:18.862124
...
def upgrade() -> None:
    ...
    op.create_index(op.f('ix_public_room_capacity'), 'room', ['capacity'], unique=False, schema='public')
    ...
    sa.Column('outcome', sa.Enum('RESERVED_ROOM', 'REQUESTED_INFORMATION', 'SUBMITTED_OH_TICKET', 'CANCELLED', name='conversationoutcome'), nullable=False),
    sa.ForeignKeyConstraint(['user_id'], ['user.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    ...
    sa.ForeignKeyConstraint(['room_id'], ['public.room.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
